 ...
 * @return {number|null}
 */
function minIndex(a: number | null, b: number | null) {
  if (a === null)
    return b
  else if (b === null)
    return a
 ...
 */
function insertInTrie(rule: any, trie: any) {
 ...
 */
function lookupInTrie(parts: string[], trie: any, index: number) {
  const part = parts[index]
  let nextNode
  let publicSuffixIndex: number | null = null
...
SuffixTrie.prototype.suffixLookup = function (hostname) {
  ...
  const publicSuffixIndex = lookupInTrie(
    parts,
    this.rules,
    parts.length - 1,
  )
  ...
  // Look for exceptions
  const exceptionIndex = lookupInTrie(
    parts,
    this.exceptions,
    parts.length - 1,
  )
